...
const SIDEBAR_WIDTH_ICON = "3.5rem" // Increased for better spacing
  ...
  (
    ...
    return (
      <aside
        ref={ref}
        className={cn(
            "group/sidebar hidden h-screen md:flex flex-col z-10 text-sidebar-foreground transition-all duration-300 ease-in-out",
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex items-center justify-between gap-2 p-2 px-3", className)}
      {...props}
    />
  )
})
  ...
  (
    ...
    if (typeof tooltip === "string") {
      tooltip = {
        ...
        className: 'bg-popover text-popover-foreground py-1 px-2 rounded-md text-xs'
      }
    }
